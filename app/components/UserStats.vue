...
const endOfSeasonDate = "2026-07-28T23:59:59-06:00";
    ...
    <div class="space-y-4 mb-5">
      <div class="grid grid-cols-2 gap-2 md:gap-4">
        ...
        <div class="bg-neutral-950/75 border border-slate-400/40 rounded px-2 py-2 sm:px-4 sm:py-4 text-center col-span-2 md:col-span-3">
          <UTabs :items="championStats" color="neutral">
            <template #mostPlayed>
              <StatsChampions v-if="data.mostPlayed" :data="data.mostPlayed" :champions="champions" />
            </template>
            <template #bestWinrate>
              <StatsChampions v-if="data.bestWinrate" :data="data.bestWinrate" :champions="champions" />
            </template>
          </UTabs>
        </div>
        <!-- ELO MÁXIMO -->
        ...
        <div class="flex flex-col gap-2 md:gap-2 col-span-2 md:col-span-1">
          ...
          <div class="bg-neutral-950/75 border border-purple-300/40 rounded p-2 sm:p-2 text-center flex flex-col items-center justify-center">
            ...
            <p class="md:text-xl text-purple-200 font-bold flex flex-wrap items-center justify-center">
              {{ (data?.history?.[0]?.snapshot_wins || 0 && data?.history?.[0]?.snapshot_losses ? ((data?.history?.[0]?.snapshot_wins || 0) / ((data?.history?.[0]?.snapshot_wins || 0) + (data?.history?.[0]?.snapshot_losses || 0)) * 100).toFixed(2) : "0.00") }}%
            </p>
          </div>
          <!-- VICTORIAS -->
